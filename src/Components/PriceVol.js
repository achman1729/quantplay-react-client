...
// the chart component showing price and volumn of a stock
// props needed are TimeSeries objects: series, seriesVolumn 
class PriceVol extends React.Component {
    constructor(props) {
        super(props) // should have props.series, props.seriesVolume of class TimeSeries, containing price and volumn data, column "close" is used
        this.state = {
    ...
    // function returning main JSX element, line-chart for price and bar-chart for volumn
    renderChart = () => {
        const { timerange } = this.state
        const croppedSeries = this.props.series.crop(timerange)
        const croppedVolumeSeries = this.props.seriesVolume.crop(timerange)
            ...
            >
                ...
                <ChartRow height="200" axisMargin={0}>
                    <Charts>
                        <BarChart
                            axis="y" 
                            style={{ volume: { normal: { fill: "#997700" } } }}
                            columns={["volume"]}
